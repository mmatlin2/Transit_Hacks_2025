#Chat GPT used for API implmentation and HTML layout, prompt: "Create a dashboard using Dash and Leaflet to visualize this Chicago transit data with interactive maps and charts."
...
# Static legend helper
def static_legend_div(color, label):
    return html.Div([
        html.B(label),
        html.Div(style={"backgroundColor": color, "width": "20px", "height": "20px",
...
bus_cmap     = LinearColormap(["lightblue","darkblue"],
    vmin=bus_df["count_per_day"].min(), vmax=bus_df["count_per_day"].max())
pickup_cmap  = LinearColormap(["yellow","red"],
    vmin=pickup_df["count_per_day"].min(), vmax=pickup_df["count_per_day"].max())
dropoff_cmap = LinearColormap(["lightgreen","darkgreen"],
    vmin=dropoff_df["count_per_day"].min(), vmax=dropoff_df["count_per_day"].max())
train_cmap   = LinearColormap(["orange","red"],
    vmin=train_df["train_total_per_day"].min(), vmax=train_df["train_total_per_day"].max())
...
bus_bottom_val = bus_df['count_per_day'].quantile(0.10)
bus_top_val    = bus_df['count_per_day'].quantile(0.90)
train_bottom_val = train_df['train_total_per_day'].quantile(0.10)
train_top_val    = train_df['train_total_per_day'].quantile(0.90)

bottom_bus_df = bus_df[bus_df['count_per_day'] <= bus_bottom_val]
top_bus_df    = bus_df[bus_df['count_per_day'] >= bus_top_val]
bottom_train_df = train_df[train_df['train_total_per_day'] <= train_bottom_val]
top_train_df    = train_df[train_df['train_total_per_day'] >= train_top_val]

bottom_bus_markers = [
    dl.CircleMarker(center=(row['lat'], row['lon']), radius=FIXED_RADIUS,
                    color='purple', fillColor='purple', fillOpacity=0.7,
                    children=[dl.Tooltip(f"Bottom 10% Bus: {row['count_per_day']:.1f}")])
    for _, row in bottom_bus_df.iterrows()
]
top_bus_markers = [
    dl.CircleMarker(center=(row['lat'], row['lon']), radius=FIXED_RADIUS,
                    color='gold', fillColor='gold', fillOpacity=0.7,
                    children=[dl.Tooltip(f"Top 10% Bus: {row['count_per_day']:.1f}")])
    for _, row in top_bus_df.iterrows()
]
bottom_train_markers = [
    dl.CircleMarker(center=(row['lat'], row['lon']), radius=FIXED_RADIUS,
                    color='navy', fillColor='navy', fillOpacity=0.7,
                    children=[dl.Tooltip(f"Bottom 10% Train: {row['train_total_per_day']:.1f}")])
    for _, row in bottom_train_df.iterrows()
]
top_train_markers = [
    dl.CircleMarker(center=(row['lat'], row['lon']), radius=FIXED_RADIUS,
                    color='orange', fillColor='orange', fillOpacity=0.7,
                    children=[dl.Tooltip(f"Top 10% Train: {row['train_total_per_day']:.1f}")])
    for _, row in top_train_df.iterrows()
]
...
layers = [
    ...
    dl.LayerGroup(bottom_bus_markers, id="bottom-bus-markers"),
    dl.LayerGroup(top_bus_markers,    id="top-bus-markers"),
    dl.LayerGroup(bottom_train_markers, id="bottom-train-markers"),
    dl.LayerGroup(top_train_markers,    id="top-train-markers"),
]
...
app.layout = html.Div([
    ...
    dl.Map([
        dl.LayersControl([
            ...
            dl.BaseLayer(
                dl.TileLayer(url="https://cartocdn_basemaps.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png",
                             attribution="© CARTO"),
            ...
            dl.Overlay(layers[6], name="Bottom 10% Bus Stops", checked=False),
            dl.Overlay(layers[7], name="Top 10% Bus Stops", checked=False),
            dl.Overlay(layers[8], name="Bottom 10% Train Stations", checked=False),
            dl.Overlay(layers[9], name="Top 10% Train Stations", checked=False),
        ], position="topleft")
    ], center=CENTER, zoom=ZOOM, style={"height":"600px", "width":"100%", "margin":"auto"}),
    ...
    # Policy legends
    html.Div([
        static_legend_div('purple', "Bottom 10% Bus Stops"),
        static_legend_div('gold',   "Top 10% Bus Stops"),
        static_legend_div('navy',   "Bottom 10% Train Stations"),
        static_legend_div('orange', "Top 10% Train Stations"),
    ], style={"textAlign":"center", "marginTop":"10px"}),
    ...
    html.Div([
        html.Div([html.H4("Bus Boardings"), summary_table(bus_df["count_per_day"])], style={"marginRight":"40px"}),
        html.Div([html.H4("Taxi Pickups"), summary_table(pickup_df["count_per_day"])], style={"marginRight":"40px"}),
        html.Div([html.H4("Taxi Dropoffs"), summary_table(dropoff_df["count_per_day"])], style={"marginRight":"40px"}),
        html.Div([html.H4("Train Rides"), summary_table(train_df["train_total_per_day"])])
    ], style={"display":"flex"})
])
